...
export default class Api {
  ...
  models(){
    ...
    */
    //微拍日程
    _ret.queryAuctionScheduleList = this.setModel('/api/WeiPai/QueryAuctionScheduleList');
    //拍品图录
    _ret.getAuctionDetailedInfo = this.setModel('/api/WeiPai/GetAuctionDetailedInfo');
    //添加关注
    _ret.addFavoriteLot = this.setModel('/api/WeiPai/AddFavoriteLot');
    //取消关注
    _ret.removeFavoriteLot = this.setModel('/api/WeiPai/RemoveFavoriteLot');
    //我的关注
    _ret.getUserFavoriteLotList = this.setModel('/api/WeiPai/GetUserFavoriteLotList');
    //拍品详细
    _ret.getLotDetailedInfo = this.setModel('/api/WeiPai/GetLotDetailedInfo');
    //商户信息
    _ret.getMerchantInfo = this.setModel('/api/Merchant/GetMerchantInfo');
    //获取注册信息
    _ret.canUserApplyMerchant = this.setModel('/api/Merchant/CanUserApplyMerchant');
    //实名认证之后的注册
    _ret.submitMerchantApply = this.setModel('/api/Merchant/SubmitMerchantApply');
    //带实名认证的注册
    _ret.submitMerchantWithRealNameApply = this.setModel('/api/Merchant/SubmitMerchantWithRealNameApply');
    //确认拍品订单详情
    _ret.getLotConfirmInfo = this.setModel('/api/WeiPai/GetLotConfirmInfo');
    //提交确认单
    _ret.lotConfirmed = this.setModel('/api/WeiPai/LotConfirmed');
    //查询得标记录
    _ret.getUserWinLotList  = this.setModel('/api/WeiPai/GetUserWinLotList');
    //得标详情
    _ret.getWinLotDetailedInfo  = this.setModel('/api/WeiPai/GetWinLotDetailedInfo');
    //收款信息
    _ret.getMerchantBankAccountList  = this.setModel('/api/Merchant/GetMerchantBankAccountList');
    //广告列表
    _ret.getAdvertisementList  = this.setModel('/api/Common/GetAdvertisementList');
    //确认收货
    _ret.lotReceived  = this.setModel('/api/WeiPai/LotReceived');
    /*
    *送评系列
    */
    //获取送评单详情
    _ret.getGradingOrderDetailedInfo = this.setModel('/api/SongPing/GetGradingOrderDetailedInfo')
    //获取用户送评单列表
    _ret.getUserGradingOrderList = this.setModel('/api/SongPing/GetUserGradingOrderList')
    return _ret;
  }
  //发送请求
  /*
